...
2. [Configure push messaging on the PingOne Portal](#2-configure-push-messaging-on-the-pingone-portal)
    1. [FCM Push Notification](#21-fcm-push-notification)
    2. [HMS Push Notification](#22-hms-push-notification)
3. [Add the PingOne MFA SDK component into your existing project](#3-add-the-pingone-mfa-sdk-component-into-your-existing-project)
4. [Configure the SDK](#4-configure-the-sdk)
...
```
Ensure your application's ProGuard configuration includes these rules before building a release.
...
See [Add Firebase to your Android project](https://firebase.google.com/docs/android/setup).
...
See [Integrating Push Kit](https://developer.huawei.com/consumer/en/codelabsPortal/carddetails/HMSPushKit).

## 2 Configure push messaging on the PingOne Portal

### 2.1 FCM Push Notification
...
## 3 Add the PingOne MFA SDK component into your existing project
...
You must configure the SDK at least once before using it. Configure the SDK using `PingOne.configure()`, which sets the PingOne geo that the SDK will use as the "trust issuer". Note that you only specify one geo for the `configure` method, but this makes it possible to pair the device in multiple geographic regions.

 ```java
public static void configure(Context context, PingOneGeo geo, PingOneSDKCallback callback);
```

The PingOneGeo is an enum, see below:

```java
enum class PingOneGeo {
    /**
     * North America geographic region for connecting to the PingOne data center.
     */
    NORTH_AMERICA,

    /**
     * Europe geographic region for connecting to the PingOne data center.
     */
    EUROPE,

    /**
     * Canada geographic region for connecting to the PingOne data center.
     */
    CANADA,
...
## 5 Working with push messages in Android
...
### 5.1 Register device token on PingOne server
...
### 5.2 Handling Push Notifications

PingOne MFA SDK will only handle push notifications that were issued by the PingOne SDK server. For other push notifications, the `PingOneSDKError` object with the code `10002, unrecognizedRemoteNotification` will be returned.
